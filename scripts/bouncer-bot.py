#!/usr/bin/env python3
"""Run the CheapBugs XMTP-to-Signal bouncer bot."""
...
from cheapbugs_bouncer.signal_cli import SignalCli
from cheapbugs_bouncer.store import BouncerStore
from cheapbugs_bouncer.token import BugzTokenClient
from cheapbugs_bouncer.xmtp_runner import run_xmtp_bouncer


def build_bot(config: BouncerConfig) -> BouncerBot:
    store = BouncerStore(config.database_path)
    store.init()
    ...
    return BouncerBot(config=config, store=store, signal=signal, token=token)
...
def main() -> int:
    ...
    try:
        asyncio.run(run_xmtp_bouncer(config, bot))
