...
- **Properties**:
  ...
  - The broker computes the default quote as `CheapBugsTreasuryVault.calculateRewardAmount(1) * ceil(days remaining until the 7-day reveal window)`, checks the authenticated buyer's existing `detailKeyPayments(reportHash, buyer)` total, and returns the details key immediately when that total already covers the current quote price. Otherwise it stores the quote by request id for 15 minutes and returns price wei, days remaining, and expiry over XMTP.
  ...
  - The broker does not trust buyer-supplied amounts. It derives the buyer from the authenticated XMTP sender, treats `buyer_address` as a consistency check, verifies the stored quote, report hash, buyer, expiry, transaction receipt success, transaction sender, transaction recipient, and `detailKeyPayments(reportHash, buyer) >= quoted price` before sending the details key for a new payment.
...
- **Test Criteria**:
  - [x] Python unit tests cover quote parsing, required authenticated XMTP sender context, spoofed buyer rejection, quote pricing, already-paid key release, payment verification, key release, and underpayment rejection.
...
- **Properties**:
  ...
  - Seller flow: implemented for early detail-key unlocks. A user on the report page requests a quote, pays the treasury vault, and receives the details key only after the broker verifies the authenticated XMTP buyer, stored quote, transaction receipt, and `detailKeyPayments` ledger. If that authenticated buyer already has enough onchain `detailKeyPayments` for the current quote price, the broker returns the key directly from the quote request.
