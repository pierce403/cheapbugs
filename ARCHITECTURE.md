...
- Integration Method: Browser-side Ethereum mainnet RPC reads in [src/lib/ens.ts](/home/pierce/projects/cheapbugs/src/lib/ens.ts), surfaced through [src/auth/thirdweb.ts](/home/pierce/projects/cheapbugs/src/auth/thirdweb.ts). Avatars use the raw ENS `avatar` text record instead of viem's `getEnsAvatar` helper so valid HTTPS/IPFS images are not dropped when an avatar host rejects `HEAD` probes.
