...
- Browser-to-broker bug submissions use strict JSON schema `cheapbugs.bug_submission.v1` from `src/xmtp/broker.ts`; the current submit form collects bug type (`0day`, `nday`, `web`, `net`, `intel`), severity, target interest, title, public summary, and private details. The browser also includes broker-triage defaults, `broker_address`, and an EIP-191 `eip191_canonical_submission_v1` signature over the canonical JSON command hash. Do not re-add repro/evidence/Signal/target kind/reference/tags/review-access-key fields unless the product direction changes.
- Browser-to-broker submissions now have broker-verified reporter command signatures, but they are not yet the final contract-verifiable EIP-712 BugBundle relay signatures. Do not let the broker create user-attributed onchain bug-index records until `CheapBugsBugIndex` has a reporter-signed broker-relay path that verifies EIP-712/ECDSA and prevents replay.
- The planned BugBundle path has the submitter generate the details key, encrypt the details into the signed BugBundle, and send the out-of-bundle key to the broker over XMTP. Do not expose a frontend review access key field on the broker path.
- The current broker IPFS path is an interim implementation: `bots/cheapbugs_broker/bugbundle.py` encrypts plaintext XMTP details into a `cheapbugs.bug_bundle.v1` carrying the verified command signature, and `bots/cheapbugs_broker/ipfs.py` pins it through the local Kubo HTTP API. Keep plaintext details out of pinned IPFS JSON.
- The submit route has an inline `#xmtp-status` indicator for XMTP wallet readiness, registration-signature progress, broker-submission signature progress, success, and failure. During external-wallet signature waits it also shows `#xmtp-signature-modal` so WalletConnect/mobile approvals are visible. Keep `submit to broker` clickable while disconnected so it can show the wallet-required status instead of doing nothing.
...
- The broker sends plain text XMTP status messages after each successful submission validation stage: JSON valid, fields well formed, reporter signature valid, target valid, credentials valid. Submission credentials use `BROKER_SUBMISSION_MIN_BUGZ` plus `BROKER_REPUTATION_BLOCKLIST`.
