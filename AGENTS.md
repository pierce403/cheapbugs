...
- Browser-to-broker bug submissions use strict JSON schema `cheapbugs.bug_submission.v1` from `src/xmtp/broker.ts`; the current submit form collects bug type (`0day`, `nday`, `web`, `net`, `intel`), severity, target interest, title, public summary, and private details. Do not re-add repro/evidence/Signal/target kind/reference/tags/review-access-key fields unless the product direction changes.
...
- The current broker IPFS path is an interim implementation: `bots/cheapbugs_broker/bugbundle.py` encrypts plaintext XMTP details into an unsigned `cheapbugs.bug_bundle.v1`, and `bots/cheapbugs_broker/ipfs.py` pins it through the local Kubo HTTP API. Keep plaintext details out of pinned IPFS JSON.
