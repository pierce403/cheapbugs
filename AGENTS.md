...
- The Python broker dependency file currently asks for `xmtp==0.1.6`, but public PyPI showed `xmtp==0.1.5` and `xmtp-bindings==0.1.5` as latest on 2026-05-18. Check the real runtime with `.venv-broker/bin/python -m pip show xmtp xmtp-bindings xmtp-agent` before assuming the pin is installed.
- `xmtp-bindings` bundles the native libxmtp build; inspect `.venv-broker/lib/python*/site-packages/xmtp_bindings/libxmtp.ref` for the bundled ref. A local stale venv had `xmtp==0.1.5`, `xmtp-bindings==0.1.5`, and `swift-bindings-1.9.0.d206831`.
- Use `python3 -m unittest discover -s bots/tests -t bots` for bot tests.
- `bots/cheapbugs_broker/xmtp_runner.py` installs temporary native compatibility shims for the old Python wrapper calls to `connect_to_backend`, `create_client`, and `FfiSubscribeError`; keep them until the published Python package and bindings signatures are aligned.
