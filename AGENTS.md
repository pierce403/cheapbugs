...
- Home/index bug-listing tables use a fixed `<colgroup>` in `src/views/home.ts` plus `.bug-listing-table` widths in `src/styles.css`; keep score/date/unlock tight, author/date/unlock nowrap, and title links visibly underlined so long titles take the flexible width without collapsing metadata columns.
...
- In the `/stake` withdraw panel, show the pending amount explicitly as `in flight` during the waiting period; do not rely only on the top metric grid to communicate how many BUGZ are pending.
