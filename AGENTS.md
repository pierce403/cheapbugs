...
- The submit route has an inline `#xmtp-status` indicator for XMTP wallet readiness, progress, success, and failure. Keep `submit to broker` clickable while disconnected so it can show the wallet-required status instead of doing nothing.
