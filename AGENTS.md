...
- Home/index bug-listing tables use a fixed `<colgroup>` in `src/views/home.ts` plus `.bug-listing-table` widths in `src/styles.css`; keep score/date/unlock tight, author/date/unlock nowrap, unlock cells right-aligned, and title links visibly underlined so long titles take the flexible width without collapsing metadata columns.
