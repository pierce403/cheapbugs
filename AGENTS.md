...
- `bots/cheapbugs_broker/xmtp_runner.py` installs temporary `xmtp==0.1.6` native compatibility shims for the old Python wrapper calls to `connect_to_backend` and `create_client`; keep them until the published Python package and bindings signatures are aligned.
