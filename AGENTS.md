...
- The frontend nav is now `index`, `about`, `submit`, `review`, `stake`, `treasury`, `token`, `patrons`, with `manage` appearing only for wallets that own at least one CheapBugs contract. Header login/session controls stay compact in the top-right header block. Do not re-add the old chain/storage/wallet/SIWE debug block to the header.
