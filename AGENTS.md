...
- The `/treasury` route uses `src/lib/treasury.ts`, `src/contracts/treasuryVault.ts`, `src/contracts/priceFeeds.ts`, and the BUGZ price paths to show treasury BUGZ, estimated USD value, and `calculateRewardAmount(1)` through `calculateRewardAmount(10)` as the per-bug range. It renders cached or placeholder data first, then fills values in a throttled background refresh. USD estimates prefer Dex Screener's Base token-pairs API, cache successful browser quotes for 10 minutes under `cheapbugs.bugzUsdQuote.v1:*`, and fall back to the Uniswap v4 BUGZ/WETH quote plus Chainlink Base ETH/USD feed `0x71041dddad3595F9CEd3DcCFBe3D1F4b0a16Bb70`; if pricing fails, keep BUGZ values visible with an informative warning.
- `tests/manage-stake.spec.ts` mocks Base RPC for owner checks, role snapshots, and bond-vault account state.
- `tests/treasury.spec.ts` mocks Dex Screener plus Base RPC for BUGZ metadata, treasury balance, treasury reward math, Uniswap v4 quotes, Chainlink ETH/USD reads, and ten-minute price-cache behavior.
