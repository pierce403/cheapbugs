...
- The `/token` route shows BUGZ and connected-wallet Base ETH balances, puts thirdweb Bridge easy-buy/Base-ETH funding before Advanced Clanker Trading, and preserves static browser-signed buy/sell swaps through the Uniswap v4 Quoter and Universal Router 2.1.1 on Base. Buy/sell and other onchain writes use the shared wallet-request modal from `src/app.ts` via `AppViewContext.runWalletAction`; the modal cancel button stops CheapBugs waiting, while the user must still reject any open wallet prompt to cancel the wallet request itself. Do not replace this with a backend buy-flow.
- The frontend nav is now `index`, `submit`, `review`, `bond`, `treasury`, `token`, `patrons`, then `about`, with `manage` appearing before `about` only for wallets that own at least one CheapBugs contract. The `bond` nav item routes to canonical `/bond`; `/stake` remains a compatibility alias for old links. Header login/session controls stay compact in the top-right header block. Do not re-add the old chain/storage/wallet/SIWE debug block to the header.
- Mobile shell/nav styling lives in `src/styles.css`: below 760px, logged-out header chrome keeps the `login` button compact in the top-right of the brand row, connected auth/session controls can still stack, and `.nav-row` becomes a two-column grid of equal-width buttons. Keep nav labels short enough for 320px-wide phones.
...
- Home/index bug-listing tables use a fixed `<colgroup>` in `src/views/home.ts` plus `.bug-listing-table` widths in `src/styles.css`; keep score/date/unlock tight, score controls centered, author/date/unlock nowrap, unlock cells right-aligned, and title links visibly underlined so long titles take the flexible width without collapsing metadata columns.
...
- The `/bond` route uses `src/contracts/bondVault.ts` and shows active bond, pending withdrawal, wallet BUGZ, allowance, level, next-level threshold, and a live countdown for delayed withdrawal step 2 without showing raw contract addresses. Legacy `/stake` still renders the same view. The add-bond form has one primary button: `approve bugz` when the entered amount exceeds current allowance, otherwise `bond bugz`. Keep copy clear that pending withdrawals remain slashable and new bonds cancel pending withdrawals.
- The `/bond` route must keep the large conduct warning from `src/views/stake.ts`: bonded users who demonstrate anti-social behavior, including spam, harassment, or criminal activity related to platform bugs, can have their bond burned immediately into the treasury. Use "bond" terminology in this warning, not "stake".
- In the `/bond` withdraw panel, show the pending amount explicitly as `in flight` during the waiting period; do not rely only on the top metric grid to communicate how many BUGZ are pending.
...
- The `/treasury` route uses `src/lib/treasury.ts`, `src/contracts/treasuryVault.ts`, `src/contracts/priceFeeds.ts`, and the BUGZ price paths to show treasury BUGZ, estimated USD value, and `calculateRewardAmount(1)` through `calculateRewardAmount(10)` as the per-bug range. It renders cached or placeholder data first, then fills values in a throttled background refresh. USD estimates prefer Dex Screener's Base token-pairs API, cache successful browser quotes for 10 minutes under `cheapbugs.bugzUsdQuote.v1:*`, keep expired quotes as a stale fallback marked `(cached)` when fresh pricing fails, and fall back to the Uniswap v4 BUGZ/WETH quote plus Chainlink Base ETH/USD feed `0x71041dddad3595F9CEd3DcCFBe3D1F4b0a16Bb70` when no cached price is available. If pricing fails entirely, keep BUGZ values visible with an informative warning. To avoid extra public-RPC pressure, treasury uses cached token metadata when present and otherwise defaults to `BUGZ`/18 instead of reading token `name`/`symbol`/`decimals` on page load.
- `tests/manage-stake.spec.ts` mocks Base RPC for owner checks, role snapshots, and bond-vault account state.
- `tests/treasury.spec.ts` mocks Dex Screener plus Base RPC for BUGZ metadata, treasury balance, treasury reward math, Uniswap v4 quotes, Chainlink ETH/USD reads, ten-minute fresh price-cache behavior, and stale price fallback after fresh pricing failures.
...
- `tests/report-detail.spec.ts` mocks Base JSON-RPC, IPFS BugBundle public metadata, ENS, and EAS GraphQL to verify the individual report route hides raw contract-suite addresses, omits low-signal hash/id metadata, keeps private details above review state, uses `unlock details` copy, and renders latest EAS verdict rows with trusted/untrusted state.
...
- Cloudflare Pages deployment uses the Git-connected `cheapbugs` Pages project from `pierce403/cheapbugs` on `main`, with build command `npm run build`, output directory `dist`, root-relative Vite base paths for `cheapbugs.net`, and hash routing plus `public/_redirects` for SPA compatibility.
- If a non-main branch needs an explicit Cloudflare Pages preview and Git-connected previews are absent, use Wrangler direct upload: first build with `VITE_BUILD_ID=<commit> VITE_BUILD_TIME=<iso> VITE_ROUTER_MODE=hash VITE_BASE_PATH=/ npm run build`, then run `wrangler pages deploy dist --project-name cheapbugs --branch <branch> --commit-hash <commit> --commit-message "<message>" --commit-dirty false`. Confirm with `wrangler pages deployment list --project-name cheapbugs --environment preview --json`. Wrangler writes local logs under `~/.config/.wrangler`, so sandboxed runs may need escalation for log-file writes.
- The `cheapbugs.net` Cloudflare zone routes the apex through a proxied CNAME to `cheapbugs.pages.dev`; do not re-add the old GitHub Pages workflow or `public/CNAME`.
- Only set `VITE_BASE_PATH` when deploying under a non-root subpath. For the production Pages custom domain, it must stay `/`.
- The header login button opens a CheapBugs wallet onboarding modal before any no-installed-wallet WalletConnect QR path. The modal offers `connect with WalletConnect`, `reset WalletConnect`, and `I don't have a crypto wallet`; the no-wallet path can generate or import an embedded CheapBugs wallet.
...
- If WalletConnect gets into a stale browser state, `src/auth/thirdweb.ts` now replaces the singleton WalletConnect wallet object and clears this origin's WalletConnect localStorage/sessionStorage keys plus `WALLET_CONNECT_V2_INDEXED_DB` after WalletConnect disconnects, failed reconnects, failed login attempts, user disconnects, or the modal `reset WalletConnect` action.
...
- WalletConnect QR/mobile pairing can use a deployment-specific `VITE_WALLETCONNECT_PROJECT_ID`; when unset, Thirdweb falls back to its bundled WalletConnect project id. If wallets report `Failed to connect to cheapbugs.net`, check whether the live bundle has actually deployed the current commit, then consider setting this Pages env var.
- WalletConnect rejection code `5000` can arrive as a plain object `{ message: "User rejected.", code: 5000 }`, not an `Error`. `src/auth/thirdweb.ts` normalizes these object-shaped errors, surfaces a clear rejection message, and skips IndexedDB deletion for ordinary user rejection; reserve stale-storage clearing for real connection/reconnect failures or the manual reset button.
- Browser console logging through `appLog` keeps warnings/errors visible, but routine info logs are opt-in with `VITE_DEBUG_LOGS=1` or `localStorage.setItem("cheapbugs.debugLogs", "1")`. `src/lib/logger.ts` also filters the known harmless Thirdweb/WalletConnect `session_request ... without any listeners` console-noise case unless debug logging is enabled.
...
- Link previews use static OpenGraph/Twitter metadata in `index.html` with `https://cheapbugs.net/og-image.png`; current card copy is "report bugs, get paid" and "shitty bugs, competitive prices". Favicon and app icons are served from `public/`. Keep fancy/report-specific preview work out of the main static app; the Farcaster miniapp lives at `../cheapbugs-alerts` and is the preferred place for dynamic or richer OG/Farcaster previews.
- The site-wide development banner is rendered from `src/app.ts`; keep launch-date copy centralized there instead of duplicating it in route views. Its visual treatment should stay orange/warning-toned, not green/success-toned. Success and ready messages, including submit `#xmtp-status`, should also use the orange theme.
...
- Report detail auto-unlock uses the onchain `detailsKey` returned by `CheapBugsBugIndex.getReport` after reveal: `src/contracts/bugIndex.ts` includes the raw `bytes32` key in `SubmissionPublic`, `src/views/report.ts` converts it to the existing base64url access-key format, stores it in `cheapbugs.report-access`, and decrypts the BugBundle without user paste. Keep the report cache reveal-aware so old pre-reveal `cheapbugs.bugIndex.v2:report:*` entries are bypassed once `revealAfter` has passed or a revealed record is missing its key.
- Report detail display in `src/views/report.ts` intentionally keeps the top table to readable fields only: date, BugBundle download, author, and human target. Do not re-add report hash/id, reporter address, mode, target/content hashes, tags, or the decrypted private submission field table unless product direction changes; the private details markdown belongs above trusted review state.
- Report detail markdown rendering uses `src/lib/markdown.ts`, a local safe subset renderer for summary/details blocks. It escapes raw HTML, allows only safe absolute `http:`, `https:`, and `mailto:` links, renders fenced code blocks with top-right copy buttons, and is covered by `tests/report-detail.spec.ts`. Do not replace summary/details rendering with raw `innerHTML` from user/IPFS content.
...
- Detail unlocks use strict JSON schema `cheapbugs.detail_unlock.v1` in `src/xmtp/broker.ts` and `bots/cheapbugs_broker/commands.py`. The browser asks for a quote; if the authenticated buyer has already paid enough according to `detailKeyPayments(reportHash,buyer)` and the broker's current price, the broker can return the details key immediately. Otherwise the browser preflights treasury BUGZ allowance, approves first only when needed, pays `CheapBugsTreasuryVault.purchaseDetailKey(reportHash, amount)`, sends the tx hash back, then stores the returned details key in the existing per-report access-key localStorage path.
- The report detail page lists early-access buyers by reading `CheapBugsTreasuryVault.detailKeyPurchaseCount()` and `detailKeyPurchaseAt(index)` through `src/contracts/treasuryVault.ts`, filtering by report hash, and aggregating repeated purchases by buyer. Keep these public Base reads serialized/cached and avoid exposing raw contract-suite addresses in the route.
- `src/views/detailUnlock.ts` skips the payment allowance preflight after a confirmed approval transaction by calling `purchaseDetailKey(..., { skipAllowancePreflight: true })`; this avoids false "larger BUGZ approval" failures when a public Base RPC lags immediately after the approval receipt. Keep the onchain purchase revert/error mapper as the final allowance guard.
...
- Broker detail-unlock verification must treat the buyer as adversarial: derive the buyer from the authenticated XMTP sender (`ctx.get_sender_address()`), use message `buyer_address` only as a consistency check, verify broker/index/treasury/chain config, use the SQLite quote by request id for new payments, verify the transaction sender/recipient/status, and require `detailKeyPayments(reportHash,buyer) >= stored_quote.price_wei` before sending the key. Quote requests may also return the key immediately when the authenticated buyer's existing treasury ledger total covers the broker's current quote price. Do not trust buyer-supplied prices or amounts.
...
- Broker settlement scoring lives in `bots/cheapbugs_broker/service.py`: for valid index-treasury payouts, a positive onchain bonded vote score from `upvoteWeight - downvoteWeight` or a positive legacy Signal support score maps directly to the maximum 10x multiplier; zero/nonpositive score maps to the base 1x multiplier, and invalid/spam maps to 0.
- Before broker settlement submits `completePayout`, `bots/cheapbugs_broker/service.py` verifies the decoded stored details key against the SQLite commitment and live onchain `detailsKeyCommitment` when available; `bots/cheapbugs_broker/bug_index.py` also preflights the onchain commitment before gas estimation. Keep this guard in place when changing payout flow.
- Index Halt is the named risk where the report at `nextPayoutIndex` cannot reveal a matching details key, blocking all later payouts. `test/CheapBugsBugIndex.t.sol` has characterization tests `test_indexHaltDisappearingBrokerWithoutDetailsKeyBlocksLaterPayouts` and `test_indexHaltMangledDetailsKeyCommitmentBlocksPayoutCursor`; the current deployed index has no skip/quarantine escape hatch, so a protocol-level fix requires redeploying or migrating the index.
...
- The Python broker uses index-ordered `CheapBugsTreasuryVault` payouts by default when the index and treasury adapters are configured and `BROKER_BUGZ_BASE_REWARD` is unset. Explicit `BROKER_BUGZ_BASE_REWARD` opts into the legacy direct ERC20 transfer path from the wallet behind `BROKER_KEY`.
...
- BUGZ trading is Base-only and uses the Clanker-created Uniswap v4 WETH/BUGZ pool key configured in `src/config/env.ts`. Buys wrap ETH in Universal Router; sells require Permit2 approval before the router can pull BUGZ. thirdweb Bridge easy-buy/onramp helpers live in `src/lib/thirdwebTrading.ts`; treat them as optional browser-only convenience and keep the direct Clanker forms as the reliable sell path, even when thirdweb sell route probes return quotes.
...
- The `/token` route auto-refreshes buy/sell quotes after a 650ms dwell on amount or slippage edits. Quote reads stay available without a connected wallet, but buy/sell submit buttons stay wallet-gated, open the shared wallet-request modal, and the trade adapters re-quote before sending. Keep the Easy Buy section above `#advanced-clanker-trading`, and keep the Base ETH gas warning non-blocking when connected balances are below `MIN_BASE_ETH_FOR_GAS`.
- The header BUGZ status starts as `bugz: loading` for connected wallets, then renders the balance or `bugz: unavailable`. Header token reads log throttled warnings for temporary Base RPC rate limits and loud console errors for non-rate-limit failures, with RPC/token context. Check `/token` and the browser console for the actual balance-read failure reason.
