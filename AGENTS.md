...
- `/manage` remains owner-only. Index admins are a separate role; the `/review` route reads `CheapBugsBugIndex.admins(account)` and lets admins see the queue and call `flagBug` for `valid`, `invalid`, or `spam` status.
