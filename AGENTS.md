...
- Home/index bug-listing tables use a fixed `<colgroup>` in `src/views/home.ts` plus `.bug-listing-table` widths in `src/styles.css`; keep score/date/unlock tight and author/date/unlock nowrap so long titles take the flexible width without collapsing metadata columns.
