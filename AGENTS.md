...
- Mobile shell/nav styling lives in `src/styles.css`: below 760px, the header stacks, auth controls go full width, and `.nav-row` becomes a two-column grid with the odd final link spanning both columns. Keep nav labels short enough for 320px-wide phones.
