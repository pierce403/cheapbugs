...
- Detail-key unlock sales use strict XMTP schema `cheapbugs.detail_unlock.v1`. The broker derives the buyer from the authenticated XMTP sender, treats message `buyer_address` as a consistency check, stores quote price/expiry in SQLite for new payments, verifies the submitted transaction receipt, and reads `CheapBugsTreasuryVault.detailKeyPayments(reportHash, buyer)` before releasing a details key. Quote requests from an already-paid authenticated buyer can return the key immediately when the treasury ledger total covers the broker's current quote price.
...
- Report detail markdown rendering is a local safe subset renderer. It escapes raw HTML and only activates safe absolute links, so report summary/details markdown must not be rendered by assigning untrusted BugBundle text directly to raw HTML.
...
- Broker-side prevention: the Python broker must reject mangled submitter keys before IPFS pinning or index publication, and it must re-check the reveal key before payout settlement. A real signed BugBundle with a mismatched out-of-bundle details key is covered by `CommandParsingTest.test_reject_real_authorized_bugbundle_mangled_details_key_before_pin`; settlement-time stored/onchain commitment mismatches are covered by `SettlementRewardTest.test_settlement_refuses_index_payout_when_stored_details_key_commitment_mismatches` and `SettlementRewardTest.test_settlement_refuses_index_payout_when_onchain_details_key_commitment_mismatches` in `bots/tests/test_broker_bot.py`.
...
- Detail-unlock buyers are treated as adversarial. The broker must reject spoofed buyer fields, wrong broker/index/treasury bindings, expired quote ids, unknown report hashes, failed or unrelated transaction hashes, and payments whose treasury ledger total is below the stored quote. For quote-time repeat unlocks, the same authenticated XMTP buyer binding applies and the ledger total must cover the broker's current quote price before the key is returned.
