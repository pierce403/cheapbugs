...
`run-broker.sh` loads a shell-compatible `.env`, prepares `.venv-broker`, installs `requirements-broker.txt`, initializes the SQLite store, and starts the broker. It expects `xmtp==0.1.5`, `xmtp-bindings==0.1.5`, and `BROKER_KEY`. `BROKER_KEY` is the broker wallet key used for the XMTP identity, bug-index publishing, and current BUGZ payout adapter. Base RPC, BUGZ token, and bug-index settings default to Base mainnet and the live CheapBugs deployment, and can be overridden with `BASE_RPC_URL`, `BUGZ_TOKEN_ADDRESS`, and `BROKER_BUG_INDEX_ADDRESS`. The broker persists XMTP installation state at `BROKER_XMTP_DB_PATH`; keep that DB across restarts. Stale installations are pruned by default with signed XMTP revocation requests, and stale inbox-mismatched DB files are archived before retrying startup. `BROKER_DRY_RUN` defaults to `1`, which prevents `publishBug`, Signal relay, and token transfers while still exercising validation and IPFS pinning; set `BROKER_DRY_RUN=0` only when the broker wallet is intentionally funded and authorized. Broker events are timestamped to stdout and `broker.log` by default; set `BROKER_LOG_PATH` to override the log file.
